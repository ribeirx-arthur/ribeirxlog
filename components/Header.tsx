...
import {
  ...
  Share2, Menu, ShieldAlert, BookOpen
} from 'lucide-react';
...
const Header: React.FC<HeaderProps> = ({
  ...
  // Detect incomplete profile for sininho badge
  const isProfileIncomplete = !profile.phone || !profile.name || profile.name === 'Usuário';
  const allNotifications = isProfileIncomplete
    ? [
        {
          id: 'profile-incomplete',
          type: 'incomplete' as const,
          title: 'Perfil incompleto',
          message: 'Complete seu perfil (telefone e nome) para personalizar seus relatórios.',
    ...
    <header className="h-20 border-b border-slate-800 bg-slate-900/50 backdrop-blur-xl flex items-center justify-between px-4 md:px-8 sticky top-0 z-[100] md:ml-64">
      ...
      <div className="flex items-center gap-6">
        <div className="flex items-center gap-1">
                      ...
                      >
                        <div className="flex gap-3">
                          <div className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 ${
                            n.type === 'payment_delay' ? 'bg-rose-500/10 text-rose-500' :
                            n.type === 'incomplete' ? 'bg-amber-500/10 text-amber-500' :
                          ...
                          <div className="space-y-1">
                            ...
                            {n.id === 'profile-incomplete' && (
                              <span className="text-[9px] text-emerald-400 font-bold flex items-center gap-1">Completar agora <ChevronRight className="w-3 h-3" /></span>
                            )}
          ...
          {/* Tutorial button — replaces old FAQ modal */}
          <div className="relative">
            <button
              onClick={onOpenTutorial}
              className="p-3 rounded-2xl transition-all text-slate-400 hover:text-white hover:bg-slate-800"
