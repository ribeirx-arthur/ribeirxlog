...
const FleetHealth: React.FC<FleetHealthProps> = ({
  ...
  const getAssetHealth = (v: Vehicle) => {
    ...
    // Calcula km rodado DESDE a última troca específica
    const kmOil = v.totalKmAccumulated - (v.lastOilChangeKm || v.lastMaintenanceKm || 0);
    const kmTire = v.totalKmAccumulated - (v.lastTireChangeKm || 0);
    const kmBrake = v.totalKmAccumulated - (v.lastBrakeCheckKm || v.lastMaintenanceKm || 0);
    const kmEngine = v.totalKmAccumulated - (v.lastEngineRevKm || 0);

    const scores = [
      Math.max(0, 100 - (kmOil / t.oilChangeKm) * 100),
      Math.max(0, 100 - (kmTire / t.tireChangeKm) * 100),
      Math.max(0, 100 - (kmBrake / t.brakeCheckKm) * 100),
      Math.max(0, 100 - (kmEngine / t.engineRevKm) * 100)
    ];
  ...
  if (view === 'report' && selectedAsset) {
    ...
    const kmOil = selectedAsset.totalKmAccumulated - (selectedAsset.lastOilChangeKm || selectedAsset.lastMaintenanceKm || 0);
    const kmTire = selectedAsset.totalKmAccumulated - (selectedAsset.lastTireChangeKm || 0);
    const kmBrake = selectedAsset.totalKmAccumulated - (selectedAsset.lastBrakeCheckKm || selectedAsset.lastMaintenanceKm || 0);
    const kmEngine = selectedAsset.totalKmAccumulated - (selectedAsset.lastEngineRevKm || 0);

    const components = [
      { id: 'oil', name: 'Óleo & Filtros', life: Math.max(0, 100 - (kmOil / t.oilChangeKm) * 100), icon: Droplet, limit: `${t.oilChangeKm.toLocaleString()} KM`, color: 'sky' },
      { id: 'tire', name: 'Pneus (Tração)', life: Math.max(0, 100 - (kmTire / t.tireChangeKm) * 100), icon: CircleDot, limit: `${t.tireChangeKm.toLocaleString()} KM`, color: 'amber' },
      { id: 'brake', name: 'Lonas de Freio', life: Math.max(0, 100 - (kmBrake / t.brakeCheckKm) * 100), icon: Zap, limit: `${t.brakeCheckKm.toLocaleString()} KM`, color: 'rose' },
      { id: 'engine', name: 'Revisão Motor', life: Math.max(0, 100 - (kmEngine / t.engineRevKm) * 100), icon: Activity, limit: `${t.engineRevKm.toLocaleString()} KM`, color: 'indigo' },
    ];
              ...
              <div key={i} className="bg-slate-900 border border-slate-800 p-8 rounded-[2.5rem] space-y-6 group hover:border-emerald-500/30 transition-all">
                ...
                <div>
                  <div className="flex justify-between items-center">
                    ...
                    <button
                      onClick={() => onResetComponent && onResetComponent(selectedAsset.id, c.id as any)}
                      className="text-[9px] font-black italic text-emerald-500 hover:text-emerald-400 uppercase tracking-widest bg-emerald-500/10 px-2 py-1 rounded-lg transition-all"
                    >
                      Resetar 100%
                    </button>
                  </div>
